...
export async function normalizeArticles(
    ...
    return rawArticles.map((raw) => {
        if (!raw.id) throw new Error('[103] Отсутствует обязательное поле (id)');
        if (!raw.title) throw new Error('[103] Отсутствует обязательное поле (title)');
        if (raw.year && (typeof raw.year !== 'number' || raw.year < 1900)) {
            throw new Error('[103] Некорректный формат поля year');
        }

        const year = raw.year !== undefined && raw.year !== null
            ? raw.year
            : new Date().getFullYear();

        if (typeof year !== 'number' || year < 1900)
            throw new Error(`[104] Некорректный формат поля year: ${year}`);
        if (raw.citations !== undefined && !Array.isArray(raw.citations))
            throw new Error('[104] Некорректный формат поля (citations must be an array)');
...
export async function loadArticlesFromFile(
    ...
    if (isLoad) {
        ...
        if (!fs.existsSync(abs)) {
            throw new Error(`[101] Файл не найден: ${filePath}`);
        }
        ...
        try {
            ...
            throw new Error(`[102] Некорректный JSON (синтаксическая ошибка): ${filePath}`);
        }

        if (!Array.isArray(raw)) throw new Error('[102] Некорректный JSON (JSON must be an array)');
