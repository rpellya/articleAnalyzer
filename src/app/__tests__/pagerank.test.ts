...
describe("PageRank – Полный набор тестов по методикам проектирования", () => {
  ...
  describe("Equivalence Partitioning (EP)", () => {
    ...
    describe("Неоптимизированные тесты (табл. 2.2 – 2.5)", () => {
      ...
      // TC 10 явная проверка EP-O-1
      test("TC10: n=3 – TESTCOVER 10", () => {
        const graph = buildSparse(
          Array(3).fill({
            id: "x",
            title: "X",
            authors: [],
        ...
        const pr = pagerankSparse(graph, 0.85, 20);
        expect(pr).toHaveLength(3);
      });

      // TC11: явная проверка EP-O-2
      test("TC11: EP-O-2 — sum of ranks ≈ 1.0", () => {
        const pr = pagerankSparse(graphSparseDangling, 0.85, 20);
        const sum = pr.reduce((s, v) => s + v, 0);
        expect(sum).toBeCloseTo(1.0, 5);
      });

      // TC12: явная проверка EP-O-3
      test("TC12: EP-O-3 — all ranks >= 0", () => {
        const pr = pagerankSparse(graphSparseDangling, 0.85, 20);
        pr.forEach((r) => expect(r).toBeGreaterThanOrEqual(0));
      });
    });

    // Оптимизированные тесты (один тест – несколько TESTCOVER)
    describe("Оптимизированные тесты (табл. 2.6)", () => {
      test("MIN-1: покрывает EP-I-1,3,6,8 + EP-O-1,2,3 + EP-P-2", () => {
        const pr = pagerankSparse(graphSparseDangling, 0.85, 20);

        // EP-O-1: длина массива = n
        expect(pr).toHaveLength(2);

        // EP-O-2: сумма рангов ≈ 1.0 (стохастический вектор)
        const sum = pr.reduce((s, v) => s + v, 0);
        expect(sum).toBeCloseTo(1.0, 5);

        // EP-O-3: все ранги неотрицательны
        pr.forEach((r) => expect(r).toBeGreaterThanOrEqual(0));
      });
      ...
      test("MIN-5: покрывает EP-I-1,3,6,9 + EP-O-1,2,3 + EP-P-1", () => {
        const pr = pagerankSparse(graphSparseNoDangling, 0.85, 20);
        expect(pr).toHaveLength(2);
        const sum = pr.reduce((s, v) => s + v, 0);
        expect(sum).toBeCloseTo(1.0, 5);
  ...
  describe("Boundary Value Analysis (BVA)", () => {
    ...
    describe("Неоптимизированные тесты (табл. 2.9)", () => {
